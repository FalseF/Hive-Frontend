...
export default function UserRolesPage() {
  ...
  const fetchRoles = async () => {
    try {
      const res = await axios.get<Role1[]>("/api/roles");
      setRoles(res.data);
  ...
  // ---------------- Handlers ----------------
  const handleSaveRole = async (role1: Role1) => {
    try {
      if (editingRole) {
        ...
        // const res = await axios.post<Role1>("/api/roles", { name: roleName });
      ...
      {selectedRole ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 items-start">
          ...
          <div className="border rounded-lg shadow bg-white">
            <h4 className="sticky p-1 top-0 text-white font-semibold text-center bg-[#337ab7]">
          ...
          {/* Permissions */}
          <div className="border rounded-lg shadow bg-white">
            <h4 className="sticky top-0 p-1 text-white font-semibold text-center bg-[#337ab7]">
                  ...
                  {rolePermissions.map((perm, index) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        className={`h-4 w-4 ${
                          isEditingPerm ? "cursor-pointer" : "cursor-not-allowed"
                        }`}
                      />
                    </li>
                  ))}
